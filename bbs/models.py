...
class Article(models.Model):
    ...
    status = models.CharField(choices=status_choice,default='published',max_length=255)
    def clean(self):
...
class Comment(models.Model):
    article = models.ForeignKey("Article",on_delete=models.CASCADE,verbose_name=u"所属文章")
    parent_comment = models.ForeignKey("self",null=True, blank=True ,related_name="my_children", on_delete=models.CASCADE)
    comment_choice = ((1,u'评论'),(2,u'点赞'))
    ...
    def clean(self):
        if self.comment_type == 1 and len(self.comment) == 0 :
            raise ValidationError(u'评论不能为空！sb')
